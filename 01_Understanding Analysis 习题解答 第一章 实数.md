...
<a id="7.6.5'">附录：$f$ 的通项公式</a>

下面开始推导具体的通项公式。

$f(m,n)$ 的值，亦或说是其序号，是先数完它前面所有的对角线，再数完它所在的对角线的列数。比如说 $f(3,2)=8$，就是因为前面有三条对角线，总共 $6$ 个数，而自己在所在对角线上为第 $2$ 个（也就是第 $2$ 列），所以总共正好是 $6+2=8$。

每条对角线上，行数与列数的和都是相等的。所以 $f(m,n)$ 前的对角线就是从和为 $2$ 到和为 $m+n-1$ 的对角线，总共 $m+n-2$ 条。而第 $n$ 条对角线上的数正好有 $n$ 个，所以就变成了等差数列的计算。最后再加上 $f(m,n)$ 所在对角线的前 $n$ 个数字，就有：

$$

f(m,n)=\displaystyle\frac{\left(m+n-1\right)\left(m+n-2\right)}{2}+n
...
Cantor 在无限集理论中的工作远远超出了定理~定理1.4.11的结论。尽管最初遭到抵制，他在这一领域的创造性和不懈努力最终引发了集合论的一场革命，并改变了数学家对无限的理解方式。

### Cantor 的对角线方法

定理~定理1.4.11 (1.4.4)的证明与 Cantor 为这一结果提供的任何论证都不同。之所以选择这个证明，是因为它直接揭示了不可数性与完备性概念之间的联系，并且因为使用嵌套区间的技术将在我们后续的工作中多次使用。
...
!!! question "练习 1.5.1"
    ...
    证明$(0,1)$不可数当且仅当 $\mathbb{R}$ 不可数。这表明定理~定理1.5.1与定理~定理1.4.11是等价的。
...
与定理~定理1.4.11类似，我们采用反证法，假设确实存在一个函数 $f : \mathbb{N} \rightarrow  \left( {0,1}\right)$ ，它是单射且满射的。对于每个 $m \in  \mathbb{N}$ ， $f\left( m\right)$ 是一个介于$0$和$1$之间的实数，我们使用十进制表示法来表示它。
...
$$
{b}_{n} = \begin{cases} 2 & {a}_{nn} \neq  2 \\ 3 & {a}_{nn} = 2. \end{cases}
$$
...
!!! question "练习 1.5.3"
    ...
    (a) 每个有理数都有一个小数展开，因此我们可以应用相同的论点来证明0到1之间的有理数集是不可数的。然而，因为我们知道 $\mathbb{Q}$ 的任何子集都必须是可数的，所以定理~定理1.5.1的证明一定有缺陷。
...
!!! question "练习 1.5.4"
    ...
    $$
    S = \left\{  {\left( {{a}_{1},{a}_{2},{a}_{3},\ldots }\right)  : {a}_{n} = 0 or 1}\right\}  .
    $$
...
提到 Kurt Gödel，不禁让人对 Cantor 工作的意义作最后的评论。Gödel 以其“不完备性定理”最为著名，这些定理涉及一般公理系统的强度。Gödel 所证明的是，任何为研究算术而创建的一致公理系统必然注定是“不完备的”，即总会存在一些命题，使公理系统因过于薄弱而无法证明。在 Gödel 极其复杂的证明的核心，有一种与定理~定理1.5.1和~定理1.5.2证明中发生的事情密切相关的操作类型。Cantor 证明方法的变体也可以在计算机科学的限制性结果中找到。“停机问题”大致询问是否存在某种通用算法，可以查看每个程序并决定该程序是否最终终止。证明这种算法不存在的核心论证使用了对角线化类型的构造。主要观点是，不仅Cantor定理的推论深远，其论证技术也同样重要。作为这一现象的一个更直接的例子，对角线化方法在第6章中再次被使用——以一种建设性的方式——作为 Arzela-Ascoli 定理证明的关键步骤。
